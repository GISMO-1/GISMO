...
Memory now carries explicit trust metadata in addition to `source`.

Trust labels:
- `user_authored` — directly written or explicitly confirmed by the operator
- `gismo_inferred` — inferred or generated by GISMO or an LLM; not authoritative on its own
- `imported` — brought in from a file, snapshot, or import path
- `external` — came from a networked, plugin, or other external system
- `verified` — explicitly verified by a trusted review step
- `trusted` — approved for execution-relevant use
...
Rules:
- Seeing data does not make it trusted.
- `trusted` memory is the only memory that may silently influence planning/execution paths.
- `llm`, imported, and external content may be stored locally, but they stay out of trusted memory unless explicitly promoted.
- Any trust promotion MUST record an explicit reason and emit a security event.
...
- Planning/profile retrieval may filter to trusted memory only.
...
- `trust_labels_json` TEXT NOT NULL
